...
public class QuickSort implements Sorter<NISTEntry> {
    @Override
    ...
    private void quickSort(NISTEntry[] arr, int left, int right) {
        if (left < right) {
    ...
    private int partition(NISTEntry[] arr, int left, int right) {
        NISTEntry pivot = arr[right]; // Choose the pivot element
        int i = left - 1;
    ...
    private void swap(NISTEntry[] arr, int i, int j) {
        NISTEntry temp = arr[i];
        arr[i] = arr[j];
        arr[j] = temp;
    }
}
